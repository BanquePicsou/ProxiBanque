...
public class GeneratePDFController {
	...
	public String generateReleveCpt(ModelMap model, @PathVariable("id") int idPersonne) {
			...
			if (client.getListeCompte() != null) {
				...
				if (cc != null || ce != null) {
					...
					try {
						...
						document.addTitle("Relevé de comptes - " + client.getNom());

						document.open();

						Paragraph titre = new Paragraph("Relevé de comptes");
						titre.setAlignment(Element.ALIGN_CENTER);
						...
						tableauComptes.addCell("Numéro de compte");
						tableauComptes.addCell("Découvert (€)");
						tableauComptes.addCell("Taux d'interêts (%)");
		...
		model.addAttribute("lienPdf", pathGetPdf);
		if (pathGetPdf == null) {
			model.addAttribute("error", "Le client n'existe pas ou ne possède pas de comptes");
		}
